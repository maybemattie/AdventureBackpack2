...
public class SlotTool extends SlotAdventure {
    ...
    public static boolean isValidTool(ItemStack stack) {
        if (stack == null || stack.getMaxStackSize() != 1) return false;
        ...
        if (TinkersUtils.isTool(clazzName)) {
            for (String toolName : INVALID_TINKER_NAMES) if (itemName.contains(toolName)) return false;
            return true;
        }
        ...
        for (String toolName : INVALID_TOOL_NAMES) if (itemName.contains(toolName)) return false;

        for (String toolName : VALID_TOOL_NAMES) if (itemName.contains(toolName)) return true;
