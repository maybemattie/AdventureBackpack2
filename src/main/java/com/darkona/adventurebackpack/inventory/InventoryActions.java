...
public class InventoryActions {
     ...
     * @return True if the tank was filled and the resulting filled or empty container item was placed in the other
     *         slot.
     */
    public static boolean transferContainerTank(IInventoryTanks inventory, FluidTank tank, int slotIn) {
        ...
        if (FluidContainerRegistry.isFilledContainer(stackIn)) {
            ...
            if (fill > 0) // If it can accept the fluid
            {
                if (FluidContainerRegistry.getContainerCapacity(stackIn) + tank.getFluidAmount()
                        <= tank.getCapacity()) {
                    ItemStack stackOut = FluidContainerRegistry.drainFluidContainer(stackIn); // Get the empty container
                                                                                              // for the input, if
                                                                                              // there's any
        ...
        // TANK =====> CONTAINER
        else if (tank.getFluid() != null && tank.getFluidAmount() > 0
                && FluidUtils.isEmptyContainerForFluid(stackIn, tank.getFluid().getFluid())) {
                    int amount = FluidContainerRegistry.getContainerCapacity(tank.getFluid(), stackIn); // How much
                                                                                                        // fluid can
                                                                                                        // this
                                                                                                        // container
                                                                                                        // hold
