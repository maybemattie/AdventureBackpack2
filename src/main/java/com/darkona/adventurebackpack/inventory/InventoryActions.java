...
public class InventoryActions {
    ...
    public static boolean transferContainerTank(IInventoryTanks inventory, FluidTank tank, int slotIn) {
        ...
        if (FluidContainerRegistry.isFilledContainer(stackIn)) {
            int fill = tank.fill(
                    FluidContainerRegistry.getFluidForFilledItem(stackIn),
                    false); // See if the tank can accept moar fluid

            if (fill > 0) // If can accept the fluid
            {
                if (FluidContainerRegistry.getContainerCapacity(stackIn) + tank.getFluidAmount()
                        <= tank.getCapacity()) {
                    ItemStack stackOut = FluidContainerRegistry.drainFluidContainer(
                            stackIn); // Get the empty container for the input, if there's any
                    ...
                    if (inventory.getStackInSlot(slotOut) == null || stackOut == null) {
                        ...
                        return true;
                            ...
                            && stackOut.getItemDamage()
                                    == inventory.getStackInSlot(slotOut).getItemDamage()) {
        ...
        // TANK =====> CONTAINER
                ...
                && FluidUtils.isEmptyContainerForFluid(stackIn, tank.getFluid().getFluid())) {
            int amount = FluidContainerRegistry.getContainerCapacity(
                    tank.getFluid(), stackIn); // How much fluid can this container hold
            ...
            if (amount > 0 && drain.amount == amount) {
                ...
                if (inventory.getStackInSlot(slotOut) == null || stackOut == null) {
                    ...
                    inventory.setInventorySlotContentsNoSave(slotOut, stackOut);
                    return true;
                } else if (stackOut.getItem()
                                .equals(inventory.getStackInSlot(slotOut).getItem())
                        && stackOut.getItemDamage()
                                == inventory.getStackInSlot(slotOut).getItemDamage()) {
                    int maxStack = inventory.getStackInSlot(slotOut).getMaxStackSize();
                    if (maxStack > 1 && (inventory.getStackInSlot(slotOut).stackSize + 1) <= maxStack) {
    ...
    public static boolean areContainersOfSameType(ItemStack stackIn, ItemStack stackOut) {
        if (stackIn == null || stackOut == null) return false;
