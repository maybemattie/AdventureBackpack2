...
abstract class InventoryAdventure implements IInventoryTanks {
    ...
    public ItemStack decrStackSize(int slot, int quantity) {
        ...
        if (stack != null) {
            if (stack.stackSize <= quantity) setInventorySlotContents(slot, null);
            else stack = stack.splitStack(quantity);
        }
        return stack;
    }
    ...
    public ItemStack getStackInSlotOnClosing(int slot) {
        for (int s : getSlotsOnClosing()) if (slot == s) return inventory[slot];
    ...
    public ItemStack decrStackSizeNoSave(int slot, int quantity) {
        ItemStack stack = getStackInSlot(slot);
        if (stack != null) {
            if (stack.stackSize <= quantity) setInventorySlotContentsNoSave(slot, null);
            else stack = stack.splitStack(quantity);
        }
        return stack;
    }
