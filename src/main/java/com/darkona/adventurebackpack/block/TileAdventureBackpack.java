...
public class TileAdventureBackpack extends TileAdventure implements IInventoryBackpack, ISidedInventory {
    ...
    public void loadFromNBT(NBTTagCompound compound) {
        if (compound.hasKey("backpackData")) convertFromOldNBTFormat(compound);

        if (compound.hasKey("ench")) ench = compound.getTagList("ench", NBT.TAG_COMPOUND);
    ...
    public boolean updateTankSlots() {
        ...
        while (InventoryActions.transferContainerTank(this, getLeftTank(), BUCKET_IN_LEFT)) changesMade = true;
        while (InventoryActions.transferContainerTank(this, getRightTank(), BUCKET_IN_RIGHT)) changesMade = true;
        return changesMade;
    }
    ...
    public int[] getAccessibleSlotsFromSide(int side) {
        if (GeneralReference.isDimensionAllowed(worldObj.provider.dimensionId)) return MAIN_INVENTORY_SLOTS.clone();
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        if (slot <= Constants.END_OF_INVENTORY) return SlotBackpack.isValidItem(stack);
