    ...
    public Message onMessage(Message message, MessageContext ctx) {
        if (ctx.side.isServer()) {
            ...
            if (message.type == COPTER_ON_OFF || message.type == COPTER_TOGGLE) {
                ...
                if (copter != null) ServerActions.toggleCopterPack(player, copter, message.type);
                ...
                {
                    if (message.type == CYCLING_ON_OFF) ServerActions.toggleToolCycling(player, backpack);
