...
public class ClientEventHandler {
    ...
    public void mouseWheelDetect(MouseEvent event) {
        ...
        if (ConfigHandler.enableToolsCycling
                        && !Wearing.getWearingBackpackInv(player).getDisableCycling()
                        && SlotTool.isValidTool(heldItem)
                || BackpackTypes.getType(backpack) == BackpackTypes.SKELETON
                        && theItem.equals(Items.bow)) // TODO add bow case to server
        {
            ModNetwork.net.sendToServer(
                    new CycleToolPacket.CycleToolMessage(isWheelUp, CycleToolPacket.CYCLE_TOOL_ACTION));
