...
public final class TipUtils {
    ...
    public static void shiftFooter(List<String> eventTips) {
        if (GuiScreen.isCtrlKeyDown()) eventTips.add("");
        else eventTips.add(holdCtrl());
    }
    ...
    private static String holdThe(boolean button) {
        return whiteFormat(EnumChatFormatting.ITALIC + "<" + (button ? l10n("hold.shift") : l10n("hold.ctrl")) + ">");
    }
    ...
    public static String inventoryTooltip(NBTTagList itemList) {
        ...
        for (int i = itemCount - 1; i >= 0; i--) {
            ...
            if (slotAtI < Constants.TOOL_UPPER) break;
            else if (slotAtI == Constants.TOOL_UPPER) toolSlotU = true;
            else if (slotAtI == Constants.TOOL_LOWER) toolSlotL = true;
            else itemCount--; // this need for correct count while GUI is open and bucket slots may be occupied
        }
    ...
    private static String mainSlotsFormat(int slotsUsed) {
        ...
        if (slotsUsed == 0) slotsFormatted = EnumChatFormatting.DARK_GRAY + slotsFormatted;
        else if (slotsUsed == Constants.INVENTORY_MAIN_SIZE) slotsFormatted = EnumChatFormatting.WHITE + slotsFormatted;
        else slotsFormatted = EnumChatFormatting.GRAY + slotsFormatted;
    ...
    private static String fluidAmountFormat(int fluidAmount, int tankCapacity) {
        ...
        if (fluidAmount == tankCapacity) amountFormatted = EnumChatFormatting.WHITE + amountFormatted;
        else if (fluidAmount == 0) amountFormatted = emptyFormat();
    ...
    private static String fluidNameFormat(FluidStack fluid) {
        ...
        else if (nameUnlocalized.contains("water")) nameFormatted += EnumChatFormatting.BLUE;
        else if (nameUnlocalized.contains("oil")) nameFormatted += EnumChatFormatting.DARK_GRAY;
        else if (nameUnlocalized.contains("fuel") || nameUnlocalized.contains("creosote"))
            nameFormatted += EnumChatFormatting.YELLOW;
        else if (nameUnlocalized.contains("milk")) nameFormatted += EnumChatFormatting.WHITE;
        else if (nameUnlocalized.contains("xpjuice")) nameFormatted += EnumChatFormatting.GREEN;
        else nameFormatted += EnumChatFormatting.GRAY;
