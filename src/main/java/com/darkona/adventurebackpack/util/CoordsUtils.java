...
public class CoordsUtils {
    ...
    public static ChunkCoordinates findBlock3D(World world, int cX, int cY, int cZ, Block block, int hRange,
            int vRange) {
    ...
    private static ChunkCoordinates checkCoordsForPlayer(IBlockAccess world, int origX, int origZ, int cX, int cY,
            int cZ, boolean except) {
        LogHelper.info("Checking coordinates in X=" + cX + ", Y=" + cY + ", Z=" + cZ);
        if (except && world.isSideSolid(cX, cY - 1, cZ, ForgeDirection.UP, true)
                && world.isAirBlock(cX, cY, cZ)
                && world.isAirBlock(cX, cY + 1, cZ)
                && !areCoordinatesTheSame2D(origX, origZ, cX, cZ)) {
    ...
    /**
     * Gets you the nearest Empty Chunk Coordinates, free of charge! Looks in two dimensions and finds a block that a:
     * can have stuff placed on it and b: has space above it. This is a spiral search, will begin at close range and
     * move out.
     *
     ...
     */
    public static ChunkCoordinates getNearestEmptyChunkCoordinatesSpiral(IBlockAccess world, int origX, int origZ,
            int cX, int cY, int cZ, int radius, boolean except, int steps, byte pass, boolean type) {
        ...
        if (steps % 2 == 0) {
            if (pass == 0) {
                for (; i <= cX + steps; i++) {
                    ChunkCoordinates coords = type ? checkCoordsForPlayer(world, origX, origZ, cX, cY, cZ, except)
                            : checkCoordsForBackpack(world, cX, cY, cZ);
                    if (coords != null) {
                        return coords;
                    }
                }
                pass++;
                return getNearestEmptyChunkCoordinatesSpiral(
                        world,
                        origX,
                        origZ,
                        i,
    ...
    public static int[] canDeploySleepingBag(World world, EntityPlayer player, int cX, int cY, int cZ, boolean isTile) {
        ...
        if (isTile) {
            ...
            if (!te.isSleepingBagDeployed()) switchBy = world.getBlockMetadata(cX, cY, cZ) & 3;
        } else {
    ...
    private static int[] getDirectionAndCoordsForSleepingBag(int switchBy, World world, int cX, int cY, int cZ) {
        ...
        switch (switchBy) {
            case 0:
                --cZ;
                if (isAirAboveSolid(world, cX, cY, cZ) && isAirAboveSolid(world, cX, cY, cZ - 1)) direction = 2;
                break;
            case 1:
                ++cX;
            ...
            case 3:
                --cX;
                if (isAirAboveSolid(world, cX, cY, cZ) && isAirAboveSolid(world, cX - 1, cY, cZ)) direction = 1;
                break;
            default:
                break;
        }
