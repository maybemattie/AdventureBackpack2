...
public class CoordsUtils {
    ...
    public static ChunkCoordinates findBlock3D(
            World world, int cX, int cY, int cZ, Block block, int hRange, int vRange) {
    ...
    private static ChunkCoordinates checkCoordsForPlayer(
            IBlockAccess world, int origX, int origZ, int cX, int cY, int cZ, boolean except) {
        LogHelper.info("Checking coordinates in X=" + cX + ", Y=" + cY + ", Z=" + cZ);
        if (except
                && world.isSideSolid(cX, cY - 1, cZ, ForgeDirection.UP, true)
     ...
     */
    public static ChunkCoordinates getNearestEmptyChunkCoordinatesSpiral(
            IBlockAccess world,
            int origX,
            int origZ,
            int cX,
        ...
        if (steps % 2 == 0) {
            if (pass == 0) {
                for (; i <= cX + steps; i++) {
                    ChunkCoordinates coords = type
                            ? checkCoordsForPlayer(world, origX, origZ, cX, cY, cZ, except)
                ...
                return getNearestEmptyChunkCoordinatesSpiral(
                        world, origX, origZ, i, cY, j, radius, except, steps, pass, type);
            }
            if (pass == 1) {
                for (; j >= cZ - steps; j--) {
                    ChunkCoordinates coords = type
    ...
    private static boolean isAirAboveSolid(World world, int cX, int cY, int cZ) {
        return world.isAirBlock(cX, cY, cZ)
                && world.getBlock(cX, cY - 1, cZ).getMaterial().isSolid();
    }
