...
public enum BackpackTypes {
    ...
    private String generateSkinName() {
        return WordUtils.capitalize(this.name().toLowerCase(), '_').replaceAll("_", "");
    }
    ...
    static {
        ...
        for (BackpackTypes type : BackpackTypes.values())
            if (byMeta.put(type, type.meta) != null) throw new IllegalArgumentException("duplicate meta: " + type.meta);
    ...
    public static BackpackTypes getType(String skinName) {
        for (BackpackTypes type : BackpackTypes.values()) {
            if (type.skinName.equals(skinName)) return type;
        }
        return UNKNOWN;
    }
    ...
    public static int getLowestUnusedMeta() {
        for (byte b = 0; b < Byte.MAX_VALUE; b++) {
            if (BY_META.get(b) == null) return b;
        }
        return -1;
    }
